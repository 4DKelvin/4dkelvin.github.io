...
(function ($) {
    $.fn.magazine = function (args) {
        ...
        this.init = function (options) {
            ...
            $layout.find(option.items).each(function (i, e) {
                $(e).attr('index', i);
            });
            //插入翻页指示
            ...
            $(window).on('hashchange', function (e) {
                ...
                $layout.find(option.items).removeClass('before-magazine');
                $layout.find(option.items).removeClass('current-magazine');
                $layout.find(option.items).removeClass('after-magazine');
                if (index - 1 >= 0) {
                    $before = $layout.find(option.items).eq(index - 1).addClass('before-magazine');
                } else {
                    $before = null;
                }
                if (index < $layout.find(option.items).length - 1) {
                    $after = $layout.find(option.items).eq(index + 1).addClass('after-magazine');
                } else {
                    $after = null;
                }
                $current = $layout.find(option.items).eq(index).addClass('current-magazine');
                $current.find('section').hide();
                setTimeout(function () {
                    $current.find('section').show();
                }, 300);
            });
        ...
        this.change = function (touchMode) {
            ...
            window.location.hash = $current.attr('index');
